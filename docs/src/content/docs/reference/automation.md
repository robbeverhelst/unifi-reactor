...
| `matching` _boolean_ | Matching is true while a state trigger's condition currently matches.<br />Absent means this Automation has never been evaluated, while false<br />means it was evaluated and the condition does not hold. Keeping those<br />apart is why this is a pointer rather than a plain bool: under<br />omitempty a plain bool's false never serialises, which read as "never<br />evaluated" and left the MATCHING printer column blank on every quiet<br />automation. |  | Optional: \{\} <br /> |
