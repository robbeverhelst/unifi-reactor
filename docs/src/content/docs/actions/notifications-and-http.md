...
**`.State` carries the keys in `spec.when.state` and nothing else** — an automation triggered on `wan` cannot read `{{ .State.isp }}`, whatever the provider observed. A reference that could never resolve is reported when the object is reconciled, as `Ready=False` with reason `TemplateWillNotRender` and a message naming the field, the key and the fix; it does not stop the automation acting on its targets. See [§12a](/troubleshooting/actions-and-targets/#12a-templatewillnotrender) for everything it reports and the three cases it deliberately cannot decide. Anything that reaches a send anyway is an error rather than the words `no value`. Both the check and the send are blind to the `index` builtin (which you need for a dotted key, `{{ index .State "ups.battery" }}`): it returns an empty string for a key that is not there.
