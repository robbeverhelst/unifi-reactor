  ...
  integrations: [
    ...
    starlight({
      ...
      sidebar: [
        ...
        {
          // Generated by `make docs` from the API types, the chart's values,
          // the metric definitions and the controller's Event reasons. CI
          // fails if these pages and their sources disagree, so nothing here
          // is edited by hand — see hack/gen-reference.sh.
          label: 'Reference',
          items: [
            { label: 'Automation API', slug: 'reference/automation' },
            { label: 'Chart values', slug: 'reference/values' },
            { label: 'Metrics', slug: 'reference/metrics' },
            { label: 'Events and conditions', slug: 'reference/events' },
          ],
        },
        {
          label: 'Contributing',
