...
.PHONY: docs
docs: crd-ref-docs ## Generate the documentation site's Reference section from the API, chart, metrics and Events.
	./hack/gen-reference.sh "$(CRD_REF_DOCS)"
...
verify-testdata: ## Fail if a committed capture still contains unsanitized values.
	./hack/verify-testdata.sh

.PHONY: test
test: manifests generate fmt vet verify-testdata setup-envtest ## Run tests.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name unifi-reactor-builder
	$(CONTAINER_TOOL) buildx use unifi-reactor-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm unifi-reactor-builder
	rm Dockerfile.cross
